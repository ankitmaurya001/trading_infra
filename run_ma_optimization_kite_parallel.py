...
def run_parallel_optimization_grid(
    data,
    short_window_range,
    long_window_range,
    risk_reward_ratios,
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # Submit all tasks
        futures = {
            executor.submit(evaluate_ma_combo_worker, args): args
...
def fetch_real_data(
    ...
    try:
        ...
        kite_fetcher = KiteDataFetcher(
            credentials=cfg.KITE_CREDENTIALS, exchange=exchange
        )
...
def generate_sample_data():
    ...
    dates = pd.date_range(
        start="2025-01-01 09:15:00", end="2025-01-31 15:30:00", freq="15min"
    )
...
def main():
    ...
    if choice == "1":
        data = fetch_real_data(
            symbol=SYMBOL, days=DAYS_TO_FETCH, interval=INTERVAL, exchange=EXCHANGE
        )
    elif choice == "2":
        data = generate_sample_data()
    elif choice == "3":
        symbol = (
            input(f"Enter symbol (e.g., TATAMOTORS, RELIANCE, TCS): ").strip().upper()
        )
        exchange = (
    ...
    workers_input = input(
        f"   Number of workers (default: all CPUs, or enter number): "
    ).strip()
    max_workers = int(workers_input) if workers_input.isdigit() else MAX_WORKERS

    # Create visualizer
    visualizer = MAOptimization3DVisualizer(
        data, trading_fee=0.0, auto_open=False, output_dir="ma_optimization_plots_kite"
    )
    ...
    visualizer.print_overall_top_neighborhood_aware_points(
        metric="composite_score", top_n=5
    )
    ...
    visualizer.create_3d_gaussian_bell_curves(
        metric="composite_score", percentile_threshold=80.0
    )

    print("   📊 Creating individual bell curves...")
    visualizer.create_individual_3d_gaussian_bell_curves(
        metric="composite_score", percentile_threshold=80.0
    )
    ...
    if os.path.exists(dashboard_path):
        ...
        print(
            f"   Plots are available in: {os.path.abspath('ma_optimization_plots_kite')}"
        )
