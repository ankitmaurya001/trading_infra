...
DEFAULT_SYMBOL = "NATGASMINI26MAYFUT"  # Default Indian stock symbol
...
DEFAULT_INTERVAL = "5m"  # Will be converted to "15minute" for Kite
...
VALIDATION_SHORT_VAL = 60  # Center value for short window range
VALIDATION_LONG_VAL = 190  # Center value for long window range
VALIDATION_RR_VAL = 9.0
VALIDATION_RANGE = 1  # Range around center values
...
VALIDATION_LONG_START = max(
    VALIDATION_LONG_VAL - VALIDATION_RANGE, VALIDATION_SHORT_VAL + VALIDATION_RANGE + 1
)
VALIDATION_LONG_END = VALIDATION_LONG_VAL + VALIDATION_RANGE
DEFAULT_VALIDATION_SHORT_RANGE = list(
    range(VALIDATION_SHORT_START, VALIDATION_SHORT_END + 1)
)
DEFAULT_VALIDATION_LONG_RANGE = list(
    range(VALIDATION_LONG_START, VALIDATION_LONG_END + 1)
)
DEFAULT_VALIDATION_RR_RANGE = [
    6.0,
    6.5,
    7.0,
    7.5,
...
DEFAULT_PARAMS = [
    {
        "short_window": VALIDATION_SHORT_VAL,
        "long_window": VALIDATION_LONG_VAL,
        "risk_reward_ratio": VALIDATION_RR_VAL,
    }
]
...
def create_cumulative_pnl_chart(
    ...
    for idx, trade in closed_trades.iterrows():
        ...
        cumulative_pnl.append(
            (running_balance - initial_balance) / initial_balance * 100
        )
    ...
    first_entry = (
        closed_trades["entry_time"].iloc[0]
...
def create_combined_pnl_chart(
    all_results: List[Dict], symbol: str, initial_balance: float, output_path: str
) -> str:
    """
    ...
    for idx, result in enumerate(all_results):
        ...
        closed_trades = trade_history[
            trade_history["status"].isin(closed_statuses)
        ].copy()
...
def _load_params_list(
    params_str: str, params_file: str, default_params: List[Dict[str, Any]] = None
) -> List[Dict[str, Any]]:
    ...
    if params_file:
        ...
        if not isinstance(parsed, list):
            raise ValueError(
                "--params-file must contain a JSON array of parameter objects"
            )
        return parsed
    ...
    raise ValueError(
        "You must provide parameter sets via --params, --params-file, or set DEFAULT_PARAMS in the script"
    )
...
def run_ma_mock(
    ...
    validation_short_range: List[
        int
    ] = None,  # Short window range for validation optimization
    validation_long_range: List[
        int
    ] = None,  # Long window range for validation optimization
    validation_rr_range: List[
        float
    ] = None,  # Risk-reward range for validation optimization
) -> Dict[str, Any]:
    ...
    if enable_parameter_validation:
        ...
        actual_start_date = (
            end_dt - timedelta(days=backtest_days + days_to_validate)
        ).strftime("%Y-%m-%d")
        print(
            f"📊 Validation enabled: Fetching extra {days_to_validate} days of past data for validation"
        )
        print(f"   Fetching data: {actual_start_date} → {end_date_kite}")
        print(
            f"   ⚠️  Backtest will ONLY use: {original_start_date_kite} → {original_end_date_kite} (no data leak)"
        )
    ...
    if max_leverage == 1.0:
        ...
        print(
            f"⚙️  Leverage is 1x: Auto-setting max_loss_percent to 100% (disables ATR-based position sizing)"
        )
    ...
    if enable_parameter_validation:
        ...
        backtest_data = data[
            data.index.map(
                lambda x: original_start_dt.date()
                <= get_date_from_index(x)
        ...
        print(
            f"   Backtest data: {len(backtest_data)} points ({original_start_date_kite} → {original_end_date_kite})"
        )
        print(
            f"   Extra past data: {len(data) - len(backtest_data)} points (for validation only)"
        )

        validator.train_data = backtest_data.iloc[:0].copy()
        validator.test_data = (
            backtest_data.copy()
        )  # Only backtest period for actual trading
        ...
        print(
            f"📅 Backtest range: {backtest_data.index[0]} to {backtest_data.index[-1]}"
        )
    else:
    ...
    for idx, params in enumerate(params_list, start=1):
        ...
        for j in range(len(validator.test_data)):
            ...
            if enable_parameter_validation and param_validator:
                ...
                if current_ts.tz is not None:
                    current_ts = current_ts.tz_localize(
                        None
                ...
                if should_validate:
                    ...
                    try:
                        ...
                        validation_start_date = current_date - timedelta(
                            days=days_to_validate
                        )
                        ...
                        validation_data_source = getattr(
                            validator, "_validation_data", validator.test_data
                        )
                        ...
                        recent_data = validation_data_source[
                            validation_data_source.index.map(
                                lambda x: validation_start_date
                                <= get_date_from_index(x)
                                <= validation_end_date
                            )
                        ].copy()
                        ...
                        if len(recent_data) < 50:  # Need minimum data points
                            print(
                                f"⚠️  Insufficient data for validation ({len(recent_data)} points), skipping..."
                            )
                            print(
                                f"   Need at least {days_to_validate} days of past data from {validation_start_date}"
                            )
                        else:
                            print(
                                f"📊 Validating on {len(recent_data)} data points from {validation_start_date} to {validation_end_date}"
                            )
                            print(
                                f"   Using {days_to_validate} days of past data for optimization"
                            )

                            # Run optimization on recent data with specified ranges
                            new_optimal_params, new_optimal_metrics = (
                                param_validator.run_optimization(
                                    recent_data,
                            ...
                            current_metrics = param_validator.evaluate_parameters(
                                recent_data, params_with_fee
                            )

                            # Calculate distance and gap
                            param_distance = (
                                param_validator.calculate_parameter_distance(
                                    params_with_fee, new_optimal_params
                                )
                            )
                            performance_gap = new_optimal_metrics.get(
                                "total_pnl", 0
                            ...
                            if (
                                param_distance
                                >= param_validator.distance_threshold_critical
                                or performance_gap
                                >= param_validator.performance_gap_threshold * 2
                            ):
                                alert_level = "critical"
                            elif (
                                param_distance
                                >= param_validator.distance_threshold_warning
                                or performance_gap
                                >= param_validator.performance_gap_threshold
                            ):
                                alert_level = "warning"
                            elif (
                                param_distance
                                >= param_validator.distance_threshold_monitor
                            ):
                                alert_level = "monitor"
                            ...
                            print(
                                f"   Current Params: Short={params_with_fee['short_window']}, Long={params_with_fee['long_window']}, RR={params_with_fee['risk_reward_ratio']}"
                            )
                            print(
                            ...
                            if should_reopt:
                                print(
                                    f"   ⚠️  RECOMMENDATION: Parameters have drifted - consider re-optimization!"
                                )
                                if alert_level == "critical":
                                    print(
                                        f"   🚨 CRITICAL: Strong recommendation to stop and re-optimize"
                                    )
                            else:
                                print(
                                    f"   ✅ Parameters still optimal - continue trading"
                                )
        ...
        if "total_pnl" in performance_metrics:
            ...
            print(
                f"📈 Total PnL: {performance_metrics['total_pnl']:.2%} (₹{total_pnl_rupees:+,.2f})"
            )
    ...
    if results:
        ...
        out_json = os.path.join(
            output_dir,
            f"ma_mock_results_{symbol}_{exchange}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
        )
        ...
        for r in results:
            ...
            if "parameter_validation_history" in r:
                result_dict["parameter_validation_history"] = r[
                    "parameter_validation_history"
                ]
        ...
        for r in results:
            if r.get("trade_history") is not None and not r["trade_history"].empty:
                ...
                try:
                    ...
                    print(
                        f"⚠️  Failed to create chart for param set {r['parameter_set']}: {e}"
                    )
    ...
    if failed_results:
        ...
        for failed in failed_results:
            print(
                f"   - Set {failed['parameter_set']}: {failed['error']} (stage: {failed['stage']})"
            )
    ...
    if enable_parameter_validation and results:
        ...
        print(
            f"Validation was performed every {validation_data_window_days} days during backtest"
        )
        print(
            f"Each validation used {days_to_validate} days of past data for optimization"
        )
        ...
        for r in results:
            if r.get("parameter_validation_history"):
                ...
                print(
                    f"   Validation checks performed: {len(r['parameter_validation_history'])}"
                )
                ...
                for val_check in r["parameter_validation_history"]:
                    ...
                    print(
                        f"   {emoji} {val_check['date']}: Distance={val_check['parameter_distance']:.2f}, Gap={val_check['performance_gap']:.2%}, Level={alert_level.upper()}"
                    )
                ...
                critical_count = sum(
                    1
                    for v in r["parameter_validation_history"]
                ...
                if critical_count > 0:
                    print(
                        f"   🚨 CRITICAL: {critical_count} critical drift(s) detected during simulation"
                    )
                elif warning_count > 0:
                    print(
                        f"   ⚠️  WARNING: {warning_count} warning(s) detected during simulation"
                    )
                else:
                ...
                print(
                    f"   ⚠️  No validation checks performed (insufficient data or validation disabled)"
                )
    ...
    return {
        ...
        "interval": kite_interval,
        "backtest_data": (
            validator.test_data.copy()
            if validator.test_data is not None
            else pd.DataFrame()
        ),
        "results": results,
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Run MA strategy mock tests with Kite data and explicit params"
    )
    p.add_argument(
        "--symbol",
        default=None,
        help=f"Symbol, e.g., TCS, RELIANCE (default: {DEFAULT_SYMBOL})",
    )
    p.add_argument(
        "--exchange",
        default=None,
        help=f"Exchange: NSE, BSE, MCX (default: {DEFAULT_EXCHANGE})",
    )
    p.add_argument(
        "--start",
        default=None,
    ...
    p.add_argument(
        "--interval",
        default=None,
        help=f"Interval, e.g., 1m, 5m, 15m (default: {DEFAULT_INTERVAL})",
    )
    p.add_argument(
        "--params",
        help="JSON array string of parameter objects (overrides DEFAULT_PARAMS)",
    )
    p.add_argument(
        "--params-file",
        help="Path to JSON file with parameter array (overrides DEFAULT_PARAMS)",
    )
    ...
    p.add_argument(
        "--mock-delay", type=float, default=0.0, help="Seconds between ticks"
    )
...
def main():
    ...
    params_list = _load_params_list(
        args.params, args.params_file, default_params=DEFAULT_PARAMS
    )
